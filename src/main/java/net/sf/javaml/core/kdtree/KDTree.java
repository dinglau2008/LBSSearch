...
public class KDTree {
	...
	public Object nearest(double[] key) {
		...
		Object[] nbrs = nearest(key, 1, Double.MAX_VALUE);
		return nbrs[0];
	}
    ...
    public Object[] nearest(double[] key, int n) {
        ...
        return nearest(key, n, Double.MAX_VALUE);
    }
	

    public Object[] withinDistance(double[] key, double maxDistance) {
        
        return nearest(key, m_count, maxDistance);
    }
	 ...
	 */
	public Object[] nearest(double[] key, int n, double maxDistance) {
...
//		double max_dist_sqd = Double.MAX_VALUE;
		...
		KDNode.nnbr(m_root, keyp, hr, maxDistance, 0, m_K, nnl);

		int pointSize = nnl.getSize();
        Object[] nbrs = new Object[pointSize];
		for (int i = 0; i < pointSize; ++i) {
			KDNode kd = (KDNode) nnl.removeHighest();
			nbrs[pointSize - i - 1] = kd.v;
		}
